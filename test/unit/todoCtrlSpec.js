...
(function () {
	...
	describe('Todo Controller', function () {
		...
		it('should not have an edited Todo on start', function () {
			expect(scope.editedOrder).toBeNull();
		});
		...
		describe('having no Todos', function () {
			...
			it('should trim whitespace from new Todos', function () {
				scope.newOrder = '  buy some unicorns  ';
				scope.addTodo();
				scope.$digest();
				expect(scope.orders.length).toBe(1);
				expect(scope.orders[0].title).toBe('buy some unicorns');
			});
		});
		...
		describe('having some saved Todos', function () {
			...
			it('should count Todos correctly', function () {
				expect(scope.orders.length).toBe(5);
			...
			it('should remove Todos w/o title on saving', function () {
				var todo = store.orders[2];
				scope.editTodo(todo);
				todo.title = '';
				scope.saveEdits(todo);
				expect(scope.orders.length).toBe(4);
			});

			it('should trim Todos on saving', function () {
				var todo = store.orders[0];
				scope.editTodo(todo);
				todo.title = ' buy moar unicorns  ';
				scope.saveEdits(todo);
				expect(scope.orders[0].title).toBe('buy moar unicorns');
			});
			...
			it('revertTodo() get a Todo to its previous state', function () {
				...
				scope.$digest();
				expect(scope.orders[0].title).toBe('Uncompleted Item 0');
			});
		});
	});
}());
